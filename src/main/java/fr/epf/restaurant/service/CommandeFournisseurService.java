...
public class CommandeFournisseurService {
    ...
    public CommandeFournisseurDto changeStatut(
        ...
        commandeFournisseurRepository.updateStatut(
                commandeId, statut);
    ...
    public Collection<CommandeFournisseurDto> commandeByStatut(
            StatutCommande statut) {
        Collection<Commande> commandes = commandeFournisseurRepository.ofStatut(
                statut);
        return commandes.stream().map(this::toDto).toList();
    }

    public CommandeFournisseurDto commande(
            Long commandeId) {
        Commande commande = getCommande(commandeId);
        return toDto(commande);
    }

    public CommandeFournisseurDto addCommande(
            CommandeFournisseurRequest request) {
        boolean allIngredientExist = request.lignes()
                .stream()
                .allMatch(
                        ligneCommandeFournisseurService::checkIfIngredientExiste);

        if (!allIngredientExist) {
            throw new RuntimeException(
                    "Un ingredient n'existe pas");
        }
        Fournisseur fournisseur = fournisseurRepository.ofId(
                        request.fournisseurId())
                .orElseThrow(() -> new RuntimeException(
                        "Pas de fournisseur trouvé"));
        commandeFournisseurRepository.add(request);
        Commande commande = commandeFournisseurRepository.getLastCommandeByFournisseur(
                        request.fournisseurId())
                .orElseThrow(() -> new RuntimeException(
                        "Pas de commandes trouvé au nom de :" + fournisseur.getNom()));
        return toDto(commande);
    }

    private Commande getCommande(Long commandeId) {
        return commandeFournisseurRepository.ofId(
                        commandeId)
                .orElseThrow(() -> new RuntimeException(
                        "Commande non trouvé"));
    }

    private CommandeFournisseurDto toDto(
            Commande commande) {
        Fournisseur fournisseur = fournisseurRepository.ofId(
                        commande.getClientId())
                .orElseThrow(() -> new RuntimeException(
                        "Client non trouvé"));
        return new CommandeFournisseurDto(commande.getId(),
                fournisseur, commande.getDateCommande(),
                commande.getStatut(),
                ligneCommandeFournisseurService.getDto(
                        commande.getId()));
    }
